...
class FTReport():
	...
	# 	scatter_data = {}
	# 	scatter_list = []
	# 	ws = self.wb.create_sheet(device)
	...
	# 			boxplot_data[para].append(list(df.loc[(df[LOT] == lot), para]))

	# 		scatter_data[lot] = {
	# 			'X_DATA': df.loc[(df[LOT] == lot), 'VTH5'], 
	# 			'Y_DATA': df.loc[(df[LOT] == lot), 'RDON4'], 
	# 		}

	# 		img_path = self._drawScatter(
	# 			df.loc[(df[LOT] == lot), 'VTH5'], 
	# 			df.loc[(df[LOT] == lot), 'RDON4'], 
	# 			lot)
	# 		print(f'[Draw Figure]: Drawing the VTH-RDSON scatter figure of lot({lot})')
	# 		scatter_list.append(img_path)
	...
	# 		boxplot_list.append(img_path)

	# 	boxplot_step = 0	
	# 	for img in boxplot_list:
	# 		img_stream = openpyxl.drawing.image.Image(img)
	# 		ws.add_image(img_stream, 'A' + str(1 + boxplot_step))
	# 		boxplot_step += 38

	# 	scatter_step = 0	
	# 	for img in scatter_list:
	# 		img_stream = openpyxl.drawing.image.Image(img)
	# 		ws.add_image(img_stream, 'Q' + str(1 + scatter_step))
	# 		scatter_step += 38
	def createImgSheet(self, ftd):
